(function()
{
  ...
  var makeElement=function(tag,classNames)
  {
    ...
    el.className=(classNames||[]).map(function(cn){return nb.prefix+cn}).join(" ");
    return el
  };
  ...
  var nb={prefix:"nb-",markdown:getMarkdown()||ident,ansi:getAnsi()||ident,sanitizer:getSanitzer()||ident,highlighter:ident,VERSION:VERSION};
  ...
  {
    this.raw=raw;this.cell=cell;this.type=raw.output_type
  };
  nb.Output.prototype.renderers={display_data:render_display_data,execute_result:render_display_data,pyout:render_display_data,pyerr:render_error,error:render_error,
    stream:function()
    {
  ...
  nb.Cell=function(raw,worksheet)
  {
    ...
    if(cell.type==="code")
    {
      ...
      var raw_outputs=(cell.raw.outputs||[]).map(function(o){return new nb.Output(o,cell)});
      cell.outputs=nb.coalesceStreams(raw_outputs)
    }
  };
  nb.Cell.prototype.renderers={
    markdown:function(ci)
    {
      var el=makeElement("div",["cell","markdown-cell"]);
      el.setAttribute("cell-id",ci);
      el.innerHTML=nb.markdown(joinText(this.raw.source));
      if(root.renderMathInElement!=null)
      {
        root.renderMathInElement(el,{delimiters:[{left:"$$",right:"$$",display:true},{left:"\\[",right:"\\]",display:true},{left:"\\(",right:"\\)",display:false},{left:"$",right:"$",display:false}]})
      }
      return el
    },
    heading:function(ci)
    ...
    code:function(ci)
    {
      ...
      cell_el.setAttribute("cell-id",ci);
      cell_el.appendChild(this.input.render());
      var output_els=this.outputs.forEach(function(o){cell_el.appendChild(o.render())});
      return cell_el
    }
  };
  nb.Cell.prototype.render=function(ci)
  {
    var el=this.renderers[this.type].call(this,ci);
    this.el=el;
    return el
  };
  nb.Worksheet=function(raw,notebook)
  {
    ...
    this.render=function()
    {
      ...
      worksheet.cells.forEach(function(c,ci)
      {
        console.log(ci)
        worksheet_el.appendChild(c.render(ci))
      });
  ...
  nb.Notebook=function(raw,config)
  {
    ...
    this.worksheets=_worksheets.map(function(ws){return new nb.Worksheet(ws,notebook)});
    this.sheet=this.worksheets[0]
  };
  nb.Notebook.prototype.render=function()
  {
    var notebook_el=makeElement("div",["notebook"]);
    this.worksheets.forEach(function(w){notebook_el.appendChild(w.render())});
    this.el=notebook_el;
    return notebook_el
  };
